...
def naiveSoftmaxLossAndGradient(
    ...
    # Calculating yHat
    yHatNum = np.exp(np.dot(outsideVectors, centerWordVec))
    yHatDenom = np.sum(yHatNum)
    yHatDist = yHatNum/yHatDenom
    # creating y True one hot
    yTrue = np.zeros_like(yHatDist)
    yTrue[outsideWordIdx] = 1
    ...
    # gradient U (n, d)
    gradOutsideVecs = np.dot(diff.transpose(), centerWordVec[np.newaxis, :])
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        ...
        lossWord, gradCenterVec, gradOutsideVecs = word2vecLossAndGradient(centerWordVec,
        outsideWordIdx,
        outsideVectors,
        dataset)
        loss += lossWord
        gradCenterVecs[centerWordInd, :] = gradCenterVecs[centerWordInd, :] + gradCenterVec
